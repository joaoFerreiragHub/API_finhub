...
- 2026-03-14: P5-MARCAS Fase 4 frontend FECHADA com portal self-service em `/marcas/portal` (overview, wallet, campanhas, afiliacao e integracoes), stack dedicada (`types` + `service` + hooks React Query + page) e cobertura unitaria em `brandPortalService.test.ts`; validacoes `yarn typecheck:p1`, `yarn lint`, `yarn test brandPortalService.test.ts --runInBand` e `yarn build` em green.
...
- 2026-03-15: P5-FIRE frontend do simulador foi atualizado com comparacao visual `whatIf` (baseline vs ajustado) e painel Monte Carlo com curva de probabilidade por horizonte + percentis `P10/P50/P90` em `/ferramentas/fire/simulador`.
- 2026-03-15: P5-FIRE backend foi endurecido com cache in-memory de Monte Carlo (TTL curto para pedidos identicos) e execucao cooperativa com yield periodico ao event loop para reduzir bloqueio em corridas maiores.
